  ...
  <div class="menu-wrapper">
    <template v-for="item in routes">
      <router-link v-if="hasOneShowingChildren(item.child) && !item.child[0].child&&!item.alwaysShow" :to="item.url" :key="item.child[0].name">
        <el-menu-item :index="item.name+'/'+item.child[0].name" :class="{'submenu-title-noDropdown':!isNest}">
          <svg-icon v-if="item.child[0]&&item.child[0].icon" :icon-class="item.child[0].icon"></svg-icon>
          <span v-if="item.child[0]&&item.child[0].display_name" slot="title">{{item.child[0].display_name}}</span>
        </el-menu-item>
      </router-link>
