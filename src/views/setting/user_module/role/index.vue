<template>
  <div class="app-container">
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              size="mini"
              @click="handleViewDataPermission(scope.$index, scope.row)"
              v-if="scope.row.name != 'super_admin'"
            >数据权限</el-button>
            <el-button
              v-if="scope.row.name == 'super_admin'"
    ...
    >
      ...
      <el-tree
        :data="permissionDialogData.list"
        v-loading="permissionLoading"
        :props="permissionDialogData.defaultProps"
        ref="tree"
        :current-node-key="permissionDialogData.checkedList"
        node-key="name"
        :check-strictly="checkStrictly"
        :default-expanded-keys="permissionDialogData.defaultCheckedList"
        :default-checked-keys="permissionDialogData.defaultCheckedList"
        show-checkbox
    ...
    <el-dialog
      ...
      :close-on-click-modal="false"
    >
      <el-input size="medium" placeholder="输入关键字进行过滤" v-model="filterDataPermissionText"></el-input>
      <el-divider></el-divider>
      <el-tree
        :data="dataPermissionDialogData.list"
        v-loading="permissionLoading"
        :props="dataPermissionDialogData.defaultProps"
        ref="dataPermissionTree"
        :current-node-key="dataPermissionDialogData.checkedList"
        node-key="name"
        :check-strictly="checkStrictly"
        :default-expanded-keys="dataPermissionDialogData.defaultCheckedList"
        :default-checked-keys="dataPermissionDialogData.defaultCheckedList"
        show-checkbox
        :filter-node-method="filterNode"
      ></el-tree>
      <div slot="footer">
...
export default {
  ...
  methods: {
      ...
      this.$refs['roleForm'].validate((valid) => {
        if (valid) {
          ...
          if (this.type == 'add') {
            ...
            updateRole(this.roleForm.id, { postData: this.roleForm }).then(
              (response) => {
                if (response.errorCode == 200) {
                  this.getList()
                  this.listLoading = false
    ...
    handleAddRolePermission() {
      ...
      var postData = {
        roleId: this.permissionDialogData.role_id,
    ...
    handleAddRoleDataPermission() {
      ...
      var postData = {
        roleId: this.dataPermissionDialogData.role_id,
        dataPermissionsAllow: checkedPermission,
      }
