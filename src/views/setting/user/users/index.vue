  ...
  <div class="app-container">
        ...
        <el-table-column label="权限分配" align="center" width="150">
          <template slot-scope="scope">
            <el-dropdown size="mini" type="warning" trigger="click">
              <el-button icon="el-icon-menu" type="success" size="mini" class="button-color-red">
                权限菜单
                <i class="el-icon-arrow-down el-icon--right"></i>
              </el-button>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item divided>
                  <el-button icon="el-icon-key" type="success" class="button-color-green" size="mini"
                    @click="handleViewPermission(scope.row)">功能权限</el-button>
                </el-dropdown-item>
                <el-dropdown-item divided>
                  <el-button icon="el-icon-unlock" type="warning" class="button-color-violet" size="mini"
                    @click="handleViewDataPermission(scope.row)">数据权限</el-button>
                </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </template>
          ...
          <template slot-scope="scope">
            <el-dropdown size="mini" type="warning" trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item divided>
                  <el-button icon="el-icon-reset" type="info" size="mini" @click="handleViewResetPassword(scope.row)">
                    重置密码</el-button>
                </el-dropdown-item>
                <el-dropdown-item divided>
                  <el-button icon="el-icon-delete" type="danger" size="mini"
                    @click="handleDeleteUser(scope.$index, scope.row)" v-show="scope.row.id != userId">删除</el-button>
                </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </template>
    ...
    <div class="pagination-container">
      <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange"
        layout="total, sizes,prev, pager, next,jumper" :current-page.sync="listQuery.cur_page"
        :page-size="listQuery.page_size" :page-sizes="[10,20,30,50,80,100]" :total="total"></el-pagination>
    </div>
    <!-- 重置密码 -->
    <el-dialog title="重置密码" :visible.sync="resetPasswordDialogVisible" width="500px" :close-on-click-modal="false">
      <el-form ref="roleForm" :model="resetPasswordForm" label-width="80px">
        <el-form-item label="新密码">
          <el-input type="password" v-model="resetPasswordForm.new_password" auto-complete="off" size="medium">
          </el-input>
        </el-form-item>
        <el-form-item label="确认密码">
          <el-input type="password" v-model="resetPasswordForm.confirm_password" auto-complete="off" size="medium">
          </el-input>
        </el-form-item>
      </el-form>
      <div slot="footer">
...
export default {
  ...
  methods: {
    ...
    handleEditUser(index, row) {
      console.log(row);
      this.userDetailDialogData.userDetailDialogVisible = true;
      this.userDetailDialogData.userDetailTitle =
        '修改 "' + row.desc + '" 用户';
