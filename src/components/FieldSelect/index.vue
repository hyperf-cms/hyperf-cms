  ...
  >
    <div class="filter-container">
      ...
      <div class="selected-fields">
        <div class="box-head">已选择指标（拖拽调整显示顺序，不同模块之间不能调换指标顺序）</div>
        <div class="selected-box" ref="selectedBox">
          <div
            class="selected-group"
            v-for="(group, index) in selectedFieldsKeysOrder"
            :key="index"
            :data-pid="group"
          >
            <div
              class="selected-item"
              v-for="(item, i) in selectedFields[group]"
              :key="i"
            >
              <span>{{item.display_name}}</span>
              <div class="cancelItemBtn" @click="deleteSelected(item, i)"></div>
            </div>
          </div>
        </div>
      </div>
      <el-button style="margin-top: 10px;float: right;" @click="closeDialog">取消</el-button>
      <el-button type="primary" class="button-color-violet" style="margin: 10px 0 0 0;" @click="handleClearStorage()" v-if="cacheField">清除指标缓存</el-button>
      <el-button type="primary" style="margin: 10px 10px 0 0;float: right;" @click="handleConfirm">确认</el-button>
    </div>
  </el-dialog>
</template>
...
export default {
  ...
  created() {
      ...
      for (let i = 0; i < fieldLen; i++) {
        ...
        for (let j = 0; j < childLen; j++) {
          if (this.fields[i].children[j].name == checked) {
            ...
            if (this.selectedFields[this.fields[i].children[j].pid] == undefined) {
              this.selectedFields[this.fields[i].children[j].pid] = [];
              this.selectedFieldsKeysOrder.push(this.fields[i].children[j].pid);
            }
            this.selectedFields[this.fields[i].children[j].pid].push(this.fields[i].children[j]);
    ...
    // 判断是否全选状态
    for (let i = 0; i < this.fields.length; i++) {
      if (this.fields[i].initCheckedNum > 0 && this.fields[i].initCheckedNum == this.fields[i].children.length) {
  ...
  methods: {
    initMove() { // 初始化排序功能
      let el = this.$el.querySelectorAll('.selected-group');
      let _this = this;
      el.forEach(e => {
        new Sortable(e, {
          ghostClass: 'selected-item--active',
          onUpdate:function(event){
            const pid = event.target.dataset.pid;
            let newIndex = event.newIndex;
            let oldIndex = event.oldIndex;

            let item = _this.selectedFields[pid].splice(oldIndex, 1);
            _this.selectedFields[pid].splice(newIndex, 0, item[0]);

            // 更新返回数据
            _this.updateReturnData();
          }
        })
      })
    },
    ...
    addedFieldId(oldArr, newArr) { // 找出新增的id
      return newArr.filter(val => {
        return oldArr.indexOf(val) === -1;
      })
    },
    removedFieldId(oldArr, newArr) { // 找出删除的id
    ...
    handleCheckedFieldChange(group) {
      if (this.addArr.length > 0) { // 增加字段的情况
      ...
      if (this.removeArr.length > 0) { // 删除字段的情况
    ...
    handleClearStorage() {
      this.$confirm("确认清除指标缓存吗?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      })
        .then(() => {
          removeStore({ name: this.cacheField });

          this.$emit('getList', 'clearCache');
    ...
    deleteSelected(data, index) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        for (let j = 0; j < childLen; j++) {
          if (this.checkedKeys.indexOf(this.fields[i].children[j].id) > -1) continue fieldLoop;
        }
